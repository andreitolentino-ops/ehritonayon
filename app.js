...
function showToast(message, type='info', timeout=3500){
  try{
    ...
    // Force high-contrast styling inline to avoid caching/specificity issues
    card.style.background = 'rgba(15,23,42,0.98)';
    card.style.color = '#fff';
    card.style.borderLeft = '6px solid ' + (type==='success' ? 'rgba(34,197,94,0.9)' : type==='error' ? 'rgba(239,68,68,0.9)' : 'rgba(14,165,233,0.95)');
...
auth.onAuthStateChanged(async user => {
  ...
  if(user){
    ...
    if (ENABLE_DEMO_PATIENTS) {
      // show local demo patients so the UI isn't empty for guests
      showLocalDemoPatients();
    }
  }
...
function applyRoleUI(){
  ...
  // Show/hide Add Patient button by role
  const addPatientBtn = document.getElementById('btnAddPatient');
  if (addPatientBtn) {
    // Show for all roles (including guest); Save handler enforces auth
    const canAdd = ['admin','admission','bednav','doctor','nurse','guest'].includes(currentRole);
    addPatientBtn.style.display = canAdd ? '' : 'none';
  }

  // Gate Assign Bed / Discharge / Print by role
  const assignBtn = document.getElementById('btnAssignBed');
  if(assignBtn){
    const canAssign = ['admin','admission','bednav'].includes(currentRole);
    assignBtn.style.display = canAssign ? '' : 'none';
  }
  const dischargeBtn = document.getElementById('btnDischarge');
  if(dischargeBtn){
    const canDischarge = ['admin','doctor','admission','bednav'].includes(currentRole);
    dischargeBtn.style.display = canDischarge ? '' : 'none';
  }
  const printBtn = document.getElementById('btnPrintSummary');
  if(printBtn){
    const canPrint = ['admin','doctor','nurse','admission','bednav'].includes(currentRole);
    printBtn.style.display = canPrint ? '' : 'none';
  }

  // Gate Transfer / Wristband by role
  const transferBtn = document.getElementById('btnTransfer');
  if(transferBtn){
    const canTransfer = ['admin','doctor','admission','bednav'].includes(currentRole);
    transferBtn.style.display = canTransfer ? '' : 'none';
  }
  const wristbandBtn = document.getElementById('btnWristband');
  if(wristbandBtn){
    const canWristband = ['admin','doctor','nurse','admission','bednav'].includes(currentRole);
    wristbandBtn.style.display = canWristband ? '' : 'none';
  }
}
...
async function loadPatients(){
  ...
  if(!selectedPatientId && patients.length) { selectedPatientId = patients[0].id; try{ window.selectedPatientId = selectedPatientId; }catch(_){} }
...
// Local demo patients for guest users (not persisted)
function showLocalDemoPatients(){
  try {
    const demo = [
      { id:'demo_1', name:'John Doe', mrn:'D-1001', dob:'1985-03-14', gender:'Male', physician:'Smith', patientStatus:'Inpatient', roomNo:'201A', bedNumber:'201A' },
      { id:'demo_2', name:'Jane Santos', mrn:'D-1002', dob:'1992-07-02', gender:'Female', physician:'Lee', patientStatus:'Observation', roomNo:'3-OBS-4', bedNumber:'3-OBS-4' },
      { id:'demo_3', name:'Alex Kim', mrn:'D-1003', dob:'1978-11-23', gender:'Other', physician:'Patel', patientStatus:'Emergency', roomNo:'ER-05', bedNumber:'ER-05' }
    ];
    patients = demo;
    renderPatients(patients);
    updateStats();
  if(!selectedPatientId && patients.length) { selectedPatientId = patients[0].id; try{ window.selectedPatientId = selectedPatientId; }catch(_){} }
    try{ renderDashboardForPatient(selectedPatientId); }catch(e){}
    showToast('Showing demo patients (not saved to cloud)', 'info');
  } catch(err) {
    console.warn('showLocalDemoPatients failed', err);
  }
}
  ...
  patientsUnsub = db.collection('patients').orderBy('updatedAt','desc').onSnapshot(snap=>{
    ...
    if(selectedPatientId) {
      ...
      if(selectedPatient) {
        ...
        selectedPatientId = null; try{ window.selectedPatientId = null; }catch(_){}
        renderDashboardForPatient(null);
      }
    } else if(patients.length > 0) {
      // Auto-select first patient if no selection
      selectedPatientId = patients[0].id; try{ window.selectedPatientId = selectedPatientId; }catch(_){}
      renderDashboardForPatient(selectedPatientId);
    }
...
// Helper: select a patient by id and reflect in UI
function selectPatientById(id){
  if(!id) return;
  selectedPatientId = id;
  try { window.selectedPatientId = id; } catch(_) {}
  const p = patients.find(x=>x.id===id);
  try { renderDashboardForPatient(id); } catch(_) {}
  try { updatePatientBanner(p); } catch(_) {}
  // Update other sections so "bottom" tabs reflect selection immediately
  ...
  if(container){
    container.querySelectorAll('.patient-card').forEach(c=> c.classList.remove('selected','ring-2','ring-emerald-400','bg-emerald-50'));
    const card = container.querySelector(`.patient-card[data-pid="${CSS.escape(id)}"]`);
    if(card) card.classList.add('selected','ring-2','ring-emerald-400','bg-emerald-50');
  }
  // scroll banner into view so the change is obvious
  const nameEl = document.getElementById('patientName');
  if(nameEl) try { nameEl.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch(_) {}
...
function renderPatients(list){
  ...
  del.onclick = (ev)=> { ev.stopPropagation(); deletePatient(p.id); };
    ...
    card.addEventListener('keydown', (e)=>{
      if(e.key === 'Enter' || e.key === ' '){ e.preventDefault(); selectPatientById(p.id); }
    });
    
    container.appendChild(card);
  });
  
  // One-time delegated click as a safety net
  ...
  if(list && list.length > 0 && !selectedPatientId) {
    ...
    try { window.selectedPatientId = selectedPatientId; } catch(_) {}
    renderDashboardForPatient(selectedPatientId);
...
// Initialize immediately on this DOMContentLoaded (the outer handler)
// The previous nested DOMContentLoaded prevented this from running.
initializeEMRNavigation();
// Wire up EMR section buttons/links
try { wireEMRButtons(); } catch (e) { console.warn('wireEMRButtons failed early:', e); }

function initializeEMRNavigation() {
  if (window.__emrNavInited) return; // prevent duplicate wiring
  window.__emrNavInited = true;

  const navItems = document.querySelectorAll('.nav-item');

  // Direct listeners (nice-to-have)
  ...
  // Event delegation fallback (works even if above listeners fail)
  ...
  function setActiveNav(target) {
    document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active','bg-blue-50','text-blue-700','border-blue-200'));
    const current = document.querySelector(`.nav-item[data-target="${target}"]`);
    if (current) current.classList.add('active','bg-blue-50','text-blue-700','border-blue-200');
  }
...
// Helper: ensure a patient is selected before running an action. If none, prompt user.
function ensurePatientSelected(onSelected) {
  try {
    if (selectedPatientId) { onSelected && onSelected(); return; }
    const hasPatients = Array.isArray(patients) && patients.length > 0;
    const options = hasPatients
      ? patients.map(p=>`<option value="${p.id}">${esc(p.name||'Unnamed')} ${p.mrn?`(MRN ${esc(p.mrn)})`:''}</option>`).join('')
      : '';
    const html = `
      <div class="bg-white rounded-2xl p-6 w-full max-w-lg">
        <h3 class="text-lg font-bold mb-3">Select Patient</h3>
        ${hasPatients ? `
          <label class="text-sm text-slate-600">Choose from existing patients</label>
          <select id="pickPatient" class="w-full rounded-lg p-2 border mt-1 mb-4">${options}</select>
        ` : `
          <div class="text-sm text-slate-600 mb-4">No patients found. Add a new patient to continue.</div>
        `}
        <div class="flex justify-end gap-2">
          <button class="px-4 py-2 rounded bg-slate-100" onclick="closeModal()">Cancel</button>
          <button id="addNewFromPicker" class="px-4 py-2 rounded bg-emerald-600 text-white">Add Patient</button>
          ${hasPatients ? `<button id="usePickedPatient" class="px-4 py-2 rounded bg-blue-600 text-white">Continue</button>`:''}
        </div>
      </div>`;
    showModal(html);
    const addNew = document.getElementById('addNewFromPicker');
    if (addNew) addNew.addEventListener('click', ()=>{ try{ showAddPatientModal(); }catch(e){} });
    const useBtn = document.getElementById('usePickedPatient');
    if (useBtn) useBtn.addEventListener('click', ()=>{
      const sel = document.getElementById('pickPatient');
      if (!sel || !sel.value) { showToast('Please select a patient','error'); return; }
    ...
    console.warn('ensurePatientSelected failed', e);
    // fallback to toast
    showToast('Select a patient first','error');
  }
}

// Wire up EMR action buttons and quick-links
function wireEMRButtons() {
  if (window.__emrButtonsWired) return;
  window.__emrButtonsWired = true;

  // Patient banner actions
  const chartBtn = document.getElementById('btnChartReview');
  if (chartBtn) {
    chartBtn.addEventListener('click', () => {
      try { switchEMRSection('overview'); } catch(e) {}
      if (typeof showToast === 'function') showToast('Chart Review: showing Overview', 'info');
    });
  }
  ...
  if (searchBtn) {
    searchBtn.addEventListener('click', () => {
      const s = document.getElementById('searchTop');
      if (s) { s.focus(); s.select(); }
      if (typeof showToast === 'function') showToast('Search ready — type a name or MRN', 'info');
    });
  }

  // Overview quick-links
  const viewAllMeds = document.getElementById('linkViewAllMeds');
  if (viewAllMeds) {
    viewAllMeds.addEventListener('click', () => {
      try { switchEMRSection('medications'); } catch(e) {}
      if (typeof showToast === 'function') showToast('Navigated to Medications', 'success');
    });
  }

  const viewAllLabs = document.getElementById('linkViewAllLabs');
  if (viewAllLabs) {
    viewAllLabs.addEventListener('click', () => {
      try { switchEMRSection('labs'); } catch(e) {}
      if (typeof showToast === 'function') showToast('Navigated to Lab Results', 'success');
    });
  }
  ...
  if (saveNoteBtn) {
    saveNoteBtn.addEventListener('click', async () => {
      const area = document.getElementById('notesText');
      const note = area ? area.value.trim() : '';
      if (!note) { showToast('Please enter a note first', 'error'); return; }
      if (!window.selectedPatientId) { showToast('Select a patient first', 'error'); return; }
      try {
        // Save a simple note payload; non-destructive merge via saveTabData
        if (typeof saveTabData === 'function') {
          await saveTabData('notesSimple', { latestSimpleNote: note, latestSimpleNoteAt: new Date().toISOString() });
        }
  ...
  const templatesBtn = document.getElementById('templatesNoteBtn');
  if (templatesBtn) {
    templatesBtn.addEventListener('click', () => {
      if (typeof window.showMessagingTemplates === 'function') {
        window.showMessagingTemplates();
      } else {
  ...
  // Assign Bed / Discharge / Print buttons
  const assignBtn = document.getElementById('btnAssignBed');
  ...
  const transferBtn = document.getElementById('btnTransfer');
  if (transferBtn) transferBtn.addEventListener('click', ()=>{ ensurePatientSelected(()=> showPatientTransferModal()); });
  const wristbandBtn = document.getElementById('btnWristband');
  if (wristbandBtn) wristbandBtn.addEventListener('click', ()=>{ ensurePatientSelected(()=>{ try{ printWristband(); }catch(e){ window.print(); } }); });

  // Allergies and Code Status editors (click on chip)
  const allergyChip = document.getElementById('patientAllergies');
  if (allergyChip) allergyChip.addEventListener('click', showEditAllergiesModal);
  const codeChip = document.getElementById('patientCodeStatus');
  if (codeChip) codeChip.addEventListener('click', showCodeStatusModal);

  // Vitals sepsis screen
  const sepsisBtn = document.getElementById('btnSepsisScreen');
  if (sepsisBtn) sepsisBtn.addEventListener('click', runSepsisScreen);

  // Order sets
  const orderSetACS = document.getElementById('orderSetACS');
  if(orderSetACS) orderSetACS.addEventListener('click', ()=>applyOrderSet('ACS'));
  const orderSetPNA = document.getElementById('orderSetPNA');
  if(orderSetPNA) orderSetPNA.addEventListener('click', ()=>applyOrderSet('PNA'));

  // MAR add button
  const addMARBtn = document.getElementById('addMARBtn');
  if(addMARBtn){
    addMARBtn.addEventListener('click', (e)=>{
      e.preventDefault();
      const d = document.getElementById('marDrug')?.value.trim();
      const dose = document.getElementById('marDose')?.value.trim();
      const by = document.getElementById('marBy')?.value.trim() || (currentUser?.email || '');
      const notes = document.getElementById('marNotes')?.value.trim();
      if(!d || !dose){ showToast('Drug and dose are required','error'); return; }
      addMARRecord(d, dose, by, notes);
      showToast('MAR entry added','success');
    });
  }
}

function showEditAllergiesModal(){
  const p = patients.find(x=>x.id===selectedPatientId) || {};
  const html = `
    <div class="bg-white rounded-2xl p-6 w-full max-w-lg">
      <h3 class="text-lg font-bold mb-3">Edit Allergies</h3>
      <div class="text-xs text-slate-600 mb-2">Quick add:</div>
      <div class="flex flex-wrap gap-2 mb-3">
        ${['Penicillin','Sulfa','Aspirin','NSAIDs','Seafood','Peanuts'].map(a=>`<button type='button' class='algy-chip px-2 py-1 rounded bg-rose-50 border border-rose-200 text-rose-700 text-xs' data-a='${a}'>${a}</button>`).join('')}
      </div>
      <div class="grid grid-cols-2 gap-3 mb-3">
        <select id="allergySeverity" class="rounded-lg p-2 border">
          <option value="">Severity</option>
          <option>Mild</option>
          <option>Moderate</option>
          <option>Severe</option>
        </select>
        <input id="allergyReaction" class="rounded-lg p-2 border" placeholder="Reaction (e.g., rash, anaphylaxis)" />
      </div>
      <textarea id="editAllergiesText" rows="4" class="w-full rounded-lg p-3 border" placeholder="e.g., Penicillin (Severe - anaphylaxis), Sulfa (Rash)">${esc(p.allergies||'')}</textarea>
      <div class="mt-4 flex justify-end gap-2">
        <button class="px-4 py-2 rounded bg-slate-100" onclick="closeModal()">Cancel</button>
        <button id="addAllergyLine" class="px-4 py-2 rounded bg-blue-600 text-white">Add Line</button>
        <button id="saveAllergiesBtn" class="px-4 py-2 rounded bg-emerald-600 text-white">Save</button>
      </div>
    </div>`;
  showModal(html);
  // chip handlers
  document.querySelectorAll('.algy-chip').forEach(btn=> btn.addEventListener('click', ()=>{
    const a = btn.dataset.a; const area = document.getElementById('editAllergiesText'); if(area){ area.value = (area.value?area.value+'\n':'') + a; area.focus(); }
  }));
  const addLine = document.getElementById('addAllergyLine');
  if(addLine){
    addLine.addEventListener('click', ()=>{
      const sev = document.getElementById('allergySeverity')?.value || '';
      const rxn = document.getElementById('allergyReaction')?.value || '';
      const area = document.getElementById('editAllergiesText');
      if(area){
        const base = (area.value?area.value+'\n':'');
        area.value = base + `${sev?`(${sev}) `:''}${rxn?`Reaction: ${rxn}`:''}`.trim();
      }
    });
  }
  const btn = document.getElementById('saveAllergiesBtn');
  if(btn){
    btn.addEventListener('click', async ()=>{
      const value = document.getElementById('editAllergiesText')?.value || '';
      try{ await saveTabData('info', { allergies: value }); updatePatientBanner({ ...(patients.find(x=>x.id===selectedPatientId)||{}), allergies:value }); showToast('Allergies updated','success'); closeModal(); }catch(e){ showToast('Failed to save','error'); }
    });
  }
}
...
function showCodeStatusModal(){
  const p = patients.find(x=>x.id===selectedPatientId) || {};
  const current = (p.codeStatus || 'Full Code');
  const html = `
    <div class="bg-white rounded-2xl p-6 w-full max-w-sm">
      <h3 class="text-lg font-bold mb-3">Code Status</h3>
      <div class="space-y-2">
        ${['Full Code','DNR','DNI','DNR/DNI'].map(opt=>`<label class='flex items-center gap-2'><input type='radio' name='codestatus' value='${opt}' ${opt===current?'checked':''}/> <span>${opt}</span></label>`).join('')}
      </div>
      <div class="mt-4 flex justify-end gap-2">
        <button class="px-4 py-2 rounded bg-slate-100" onclick="closeModal()">Cancel</button>
        <button id="saveCodeStatusBtn" class="px-4 py-2 rounded bg-emerald-600 text-white">Save</button>
      </div>
    </div>`;
  showModal(html);
  const btn = document.getElementById('saveCodeStatusBtn');
  if(btn){
    btn.addEventListener('click', async ()=>{
      const sel = document.querySelector('input[name="codestatus"]:checked');
      const value = sel ? sel.value : current;
      try{ await saveTabData('info', { codeStatus: value }); updatePatientBanner({ ...(patients.find(x=>x.id===selectedPatientId)||{}), codeStatus:value }); showToast('Code status updated','success'); closeModal(); }catch(e){ showToast('Failed to save','error'); }
    });
  }
}

function runSepsisScreen(){
  if(!selectedPatientId) return showToast('Select a patient first','error');
  const p = patients.find(x=>x.id===selectedPatientId);
  const v = p && Array.isArray(p.vitals) && p.vitals.length ? p.vitals[p.vitals.length-1] : null;
  if(!v) return showToast('No vitals to screen','info');
  let triggers = [];
  const temp = parseFloat(v.temp); if(isFinite(temp) && (temp >= 38.3 || temp <= 36)) triggers.push('Temperature abnormal');
  const hr = parseFloat(v.pulse); if(isFinite(hr) && hr > 90) triggers.push('Tachycardia');
  const rr = parseFloat(v.rr || v.resp || v.RR); if(isFinite(rr) && rr > 20) triggers.push('Tachypnea');
  const spo2 = parseFloat(v.spo2 || v.spO2); if(isFinite(spo2) && spo2 < 92) triggers.push('Low SpO2');
  const bp = (v.bp||'').split('/')[0]; const sbp = parseFloat(bp); if(isFinite(sbp) && sbp < 100) triggers.push('Low SBP');
  // Optional WBC check if structured results exist
  try{
    const wbc = p?.structuredResults?.WBC?.value || p?.structuredResults?.wbc?.value;
    const wbcNum = parseFloat(wbc);
    if(isFinite(wbcNum) && (wbcNum > 12 || wbcNum < 4)) triggers.push('Abnormal WBC');
  }catch(e){}
  if(triggers.length >= 2){ showToast('Sepsis screen positive: ' + triggers.slice(0,3).join(', '), 'error'); }
  else { showToast('Sepsis screen negative', 'success'); }
}

function applyOrderSet(name){
  const prescriber = (currentUser && currentUser.email) || '';
  const sets = {
    'ACS': [
      { drug:'Aspirin', dosage:'325 mg', route:'PO', frequency:'once now' },
      { drug:'Nitroglycerin', dosage:'0.4 mg', route:'SL', frequency:'q5min PRN chest pain' },
      { drug:'Atorvastatin', dosage:'80 mg', route:'PO', frequency:'once daily' }
    ],
    'PNA': [
      { drug:'Ceftriaxone', dosage:'1 g', route:'IV', frequency:'q24h' },
      { drug:'Azithromycin', dosage:'500 mg', route:'PO', frequency:'once daily' }
    ]
  };
  const list = sets[name] || [];
  if(list.length===0) return showToast('No order set found','error');
  list.forEach(o=> addOrderToTable({ ...o, startDate: new Date().toISOString().slice(0,10), prescriber }));
  showToast(`${name} order set applied`, 'success');
}

function printPatientSummary(){
  if(!selectedPatientId) return window.print();
  const p = patients.find(x=>x.id===selectedPatientId);
  const w = window.open('', '_blank', 'width=800,height=600');
  const vitals = (p?.vitals||[]).slice(-5).map(v=> `${v.date||''} ${v.time||''}  T:${v.temp||''}  BP:${v.bp||''}  HR:${v.pulse||''}  SpO2:${v.spo2||v.spO2||''}`).join('\n');
  w.document.write(`<pre style="font-family: ui-monospace, SFMono-Regular, Menlo, monospace; padding:16px">
PATIENT SUMMARY
================
Name: ${esc(p?.name||'')}
MRN: ${esc(p?.mrn||'')}
Status: ${esc(p?.patientStatus||'')}
Physician: ${esc(p?.physician||'')}
Allergies: ${esc(p?.allergies||'None')}
Code Status: ${esc(p?.codeStatus||'Full Code')}

Latest Vitals:
${vitals || 'No vitals'}

Active Medications:
${(p?.medOrders||[]).map(m=>`- ${m.drug||''} ${m.dosage||''} ${m.route||''} ${m.frequency||''}`).join('\n') || 'None'}
...
async function saveMAREntry(entry){
  try{
    if(!selectedPatientId) return;
    await db.collection('patients').doc(selectedPatientId).update({
      mar: firebase.firestore.FieldValue.arrayUnion({
        timestamp: new Date().toISOString(),
        ...entry
      })
    });
  }catch(e){ console.warn('saveMAREntry failed', e); }
}

function printWristband(){
  if(!selectedPatientId) return window.print();
  const p = patients.find(x=>x.id===selectedPatientId) || {};
  const w = window.open('', '_blank', 'width=600,height=300');
  w.document.write(`
    <html><head><title>Wristband</title>
    <style>
      body { font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto; padding: 20px; }
      .band { border: 2px solid #111; padding: 12px 16px; border-radius: 8px; }
      .mrn { font-size: 28px; font-weight: 800; letter-spacing: 1px; }
      .name { font-size: 20px; font-weight: 700; margin-top: 6px; }
      .row { font-size: 14px; margin-top: 4px; }
    </style></head><body>
    <div class="band">
      <div class="mrn">MRN: ${esc(p.mrn||p.id||'')}</div>
      <div class="name">${esc(p.name||'')}</div>
      <div class="row">DOB: ${esc(p.dob||'—')}</div>
      <div class="row">Code: ${esc(p.codeStatus||'Full Code')}</div>
      <div class="row">Allergies: ${esc(p.allergies||'None')}</div>
    </div>
...
// Small helper to reflect selected patient into the banner
function updatePatientBanner(p){
  const setText = (id, text) => { const el = document.getElementById(id); if(el) el.textContent = text || ''; };
  if(!p){
    setText('patientName','Select Patient');
    setText('patientMRN','MRN: Not Selected');
    setText('patientDOB','DOB: --/--/----');
    setText('patientAge','--');
    setText('patientGender','--');
    setText('patientRoom','--');
  ...
  setText('patientMRN', 'MRN: ' + (p.mrn || p.id || '—'));
  if (p.dob) {
    const dob = new Date(p.dob);
    const dobStr = isNaN(dob.getTime()) ? p.dob : dob.toLocaleDateString();
    setText('patientDOB', 'DOB: ' + dobStr);
    // compute age if not provided
    if(!p.age && !isNaN(dob.getTime())){
      const today = new Date();
      let age = today.getFullYear() - dob.getFullYear();
      const m = today.getMonth() - dob.getMonth();
      if (m < 0 || (m === 0 && today.getDate() < dob.getDate())) age--;
      setText('patientAge', String(age));
    } else {
      setText('patientAge', p.age || '--');
    }
  } else {
    setText('patientDOB', 'DOB: --/--/----');
    setText('patientAge','--');
  }
  setText('patientGender', p.gender || '--');
  setText('patientRoom', p.roomNo || p.bedNumber || '--');
...
function showAddPatientModal(){
  const modalHtml = `
    <div class="bg-white rounded-2xl p-6 w-full max-w-lg">
      <h3 class="text-xl font-bold text-slate-800 mb-4">➕ Add Patient</h3>
      <div class="grid grid-cols-1 gap-3">
        <input id="newPatient_name" class="w-full rounded-lg p-2 border" placeholder="Full name *" />
        <input id="newPatient_mrn" class="w-full rounded-lg p-2 border" placeholder="MRN (optional)" />
        <div class="grid grid-cols-2 gap-3">
          <input id="newPatient_dob" type="date" class="w-full rounded-lg p-2 border" />
          <select id="newPatient_gender" class="w-full rounded-lg p-2 border">
        ...
        <input id="newPatient_physician" class="w-full rounded-lg p-2 border" placeholder="Attending Physician (optional)" />
      </div>
      <div class="flex gap-2 mt-5 justify-end">
        <button class="px-4 py-2 rounded-lg bg-slate-100" onclick="closeModal()">Cancel</button>
        <button id="newPatient_save" class="px-4 py-2 rounded-lg bg-emerald-600 text-white">Save</button>
      </div>
    </div>
  `;
  showModal(modalHtml);
  const btn = document.getElementById('newPatient_save');
  if(btn){
    btn.addEventListener('click', async ()=>{
      // Require sign-in for creation to avoid permission errors
      if(!currentUser){
        showToast('Please sign in to add a patient', 'error');
        const loginModalEl = document.getElementById('loginModal');
        if(loginModalEl) loginModalEl.classList.remove('hidden');
        return;
      }
      const name = document.getElementById('newPatient_name')?.value.trim();
      const mrn = document.getElementById('newPatient_mrn')?.value.trim();
      const dob = document.getElementById('newPatient_dob')?.value;
      const gender = document.getElementById('newPatient_gender')?.value;
      const physician = document.getElementById('newPatient_physician')?.value.trim();
      if(!name){ showToast('Name is required', 'error'); return; }
      try{
        // Ensure creating a new record
  selectedPatientId = null; try{ window.selectedPatientId = null; }catch(_){}
        await saveTabData('info', {
          name, mrn, dob, gender, physician,
          patientStatus: 'Inpatient',
          admissionDate: new Date().toISOString().split('T')[0]
        });
        // selectedPatientId set by saveTabData; refresh banner and UI
        const p = patients.find(x=>x.id===selectedPatientId) || { name, mrn, dob, gender, physician, patientStatus: 'Inpatient' };
        updatePatientBanner(p);
        renderDashboardForPatient(selectedPatientId);
        ...
        const msg = e && (e.message || e.code) ? `Failed to add patient: ${e.message || e.code}` : 'Failed to add patient';
        showToast(msg, 'error');
        try {
          // Offline/demo fallback: add to local state so the UI remains usable
          ...
          patients = [localPatient, ...patients];
          selectedPatientId = localId;
          renderPatients(patients);
          updatePatientBanner(localPatient);
          renderDashboardForPatient(selectedPatientId);
          showToast('Added locally (not saved to cloud)', 'info');
          closeModal();
        } catch (fallbackErr) {
          console.warn('Local fallback failed:', fallbackErr);
        }
      }
    });
  }
...
async function saveTabData(tabKey, data){
  ...
  if(!selectedPatientId){
    ...
    selectedPatientId = docRef.id; try{ window.selectedPatientId = selectedPatientId; }catch(_){}
...
function renderDashboardForPatient(patientId){
  ...
  // Reflect selection on the patient banner if present
  try { updatePatientBanner(p); } catch(e) {}
  ...
  // Reflect banner chips if present
  try { updatePatientBanner(p); } catch(e) {}
...
function addMARRecord(drug,dose,by,notes){
  ...
  saveMAREntry({ drug, dose, by, notes });
}
